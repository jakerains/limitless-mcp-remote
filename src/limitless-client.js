export class LimitlessAPIClient {
    ...
    async getLifelogs(params, timezone) {
        let actualParams;
        ...
        if (actualParams.date)
            queryParams.append('date', actualParams.date);
        if (actualParams.timezone)
            queryParams.append('timezone', actualParams.timezone);
        // Alternative time range parameters
        if (actualParams.start)
            queryParams.append('start', actualParams.start);
        if (actualParams.end)
            queryParams.append('end', actualParams.end);
        // Other parameters
        if (actualParams.cursor)
            queryParams.append('cursor', actualParams.cursor);
        if (actualParams.limit)
            queryParams.append('limit', actualParams.limit.toString());
    ...
    async searchLifelogs(query, limit = 10) {
        ...
        const filteredLogs = response.data.lifelogs.filter((log) => log.title?.toLowerCase().includes(query.toLowerCase()) ||
